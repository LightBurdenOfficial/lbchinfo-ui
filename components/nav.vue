  ...
  <nav class="navbar">
    <div class="navbar-brand is-size-4">
      <nuxt-link to="/" class="navbar-item navbar-logo">
        <span class="lbch-icon lbch-icon--lbch" /> explorer.lightburden.net
      </nuxt-link>
    ...
    <div class="navbar-menu" :class="{'is-active': showMenu}">
      ...
      <form class="navbar-end" @submit.prevent="search">
        <div class="navbar-item input-item">
          ...
          <button type="submit" class="button is-lbch" :class="{'is-loading': searching}">
            <Icon icon="search" />
          </button>
        </div>
      </form>
...
<script>
  import {get as lbchinfoGet} from '@/services/lbchinfo-api'
  ...
  export default {
    ...
    methods: {
      async search() {
        ...
        this.searching = true
        try {
          let {type, id, address} = await lbchinfoGet(`/search`, {params: {query: searchString}})
          switch (type) {
          case 'address':
            this.searchString = ''
