<template>
  <div class="columns is-multiline transaction-item">
        ...
        <div v-for="input in inputs" class="is-clearfix">
          ...
          <div class="script">
            ...
            <div v-if="input.scriptSig.asm">
              ...
              <code class="value"><!--
                -->{{ input.scriptSig.asm | lbch-script }}<!--
              --></code>
            </div>
          ...
          <div class="script">
            ...
            <div v-if="output.scriptPubKey.asm">
              ...
              <code class="value" :class="{'script-code': contractInfo[index]}"
                ...
                -->{{ output.scriptPubKey.asm | lbch-script }}<!--
              --></code>
            </div>
    ...
    <template v-for="({address, name, symbol, decimals, from, to, value}, index) in lpp20TokenTransfers">
      <div class="column is-full flex-full"></div>
      <AttributeInjector
        class="column collapse token-transfer-list"
        :class="{
          'first-item': index === 0,
          'last-item': index === lpp20TokenTransfers.length - 1
        }">
        <div class="is-clearfix">
    ...
    <template v-for="({address, name, symbol, from, to, tokenId}, index) in lpp721TokenTransfers">
      <div class="column is-full flex-full"></div>
      <AttributeInjector
        class="column collapse token-transfer-list"
        :class="{
          'first-item': index === 0,
          'last-item': index === lpp721TokenTransfers.length - 1
        }">
        <div class="is-clearfix">
          <AddressLink v-if="from" :address="from" class="is-pulled-left" :highlight="highlightAddress" />
          <template v-else>{{ $t('contract.token.mint_tokens') }}</template>
    ...
    <div class="column is-full has-text-right collapse-bottom" v-if="fees !== '0'">
      <template v-if="fees > 0">
        {{ $t('transaction.fee') }} <span class="amount fee">{{ fees | lbch }} LBCH</span>
      </template>
      <template v-else-if="fees < 0">
        {{ $t('transaction.reward') }} <span class="amount fee">{{ -fees | lbch }} LBCH</span>
      </template>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      lpp20TokenTransfers() {
        return this.transaction.lpp20TokenTransfers
      },
      lpp721TokenTransfers() {
        return this.transaction.lpp721TokenTransfers
      },
      contractInfo() {
