<template>
  <section class="container">
        ...
        <div class="columns">
          ...
          <div class="column info-value monospace">
            ...
            <span v-if="unconfirmed !== '0' && staking !== '0'">
              ({{ unconfirmed | lbch }} LBCH {{ $t('address.unconfirmed') }},
              {{ staking | lbch }} LBCH {{ $t('address.staking') }})
            </span>
    ...
    <nuxt-child :tokens="lpp20Balances.map(({address, name, symbol}) => ({address, name, symbol}))" />
  </section>
</template>
